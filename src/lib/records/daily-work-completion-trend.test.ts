...
describe("Daily Check Work Completion trend", () => {
  ...
  it("includes normalized targets completed after an earlier result", () => {
    ...
    expect(buildDailyWorkCompletionTrend({ ...input, checks: [] })).toEqual([
      { date, state: "available", completedWork: 1, requiredWork: 2, percentage: 50 },
    ]);
    expect(buildDailyWorkCompletionTrend({
      ...input,
      checks: [{ shift_date: date, unit_id: "unit-1", target_type: "kit", target_id: "target-1", compartment_id: null, unit_kit_id: null, status: "completed" }],
    })).toEqual([{ date, state: "available", completedWork: 2, requiredWork: 2, percentage: 100 }]);
  });
