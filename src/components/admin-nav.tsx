...
const TOP_LINKS = [
  ...
  { href: "/admin/followup", label: "Needs Follow-up" },
...
function linkClasses(href: string, pathname: string) {
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav className="flex flex-wrap gap-2">
          {TOP_LINKS.map(({ href, label }) => (
            <Link key={href} className={linkClasses(href, pathname)} href={href}>{label}</Link>
          ))}
          <div className="relative">
            <button
              ref={buttonRef}
              aria-expanded={menuOpen}
              className={`rounded-2xl px-4 py-3 font-bold shadow-sm ${menuOpen || isAdminActive(pathname) ? "bg-red-700 text-white" : "border border-slate-300 bg-white text-slate-950"}`}
            ...
            {menuOpen && (
              <div ref={menuRef} className="absolute right-0 top-full mt-1 flex w-40 flex-col gap-1 rounded-2xl border border-slate-200 bg-white p-2 shadow-lg">
