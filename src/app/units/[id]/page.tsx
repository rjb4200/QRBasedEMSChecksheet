...
export default async function UnitDashboardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: unit }, { data: checks }, { data: crew }, { data: comment }, { data: sectionComments }, addressedRows, manualItemsData] = await Promise.all([
    supabase.from("units").select("id, name, status, monthly_check_day, oos_at, oos_by, unit_compartments(id, name, sort_order, qr_location_note, unit_compartment_items(id, equipment_id, par_level, input_type)), unit_kits(id, sort_order, qr_location_note, kits(id, name, kit_items(id, equipment_id, par_level, input_type)))").eq("id", id).is("deleted_at", null).single(),
      ...
      <section className="mx-auto max-w-5xl space-y-5">
        ...
        {unit?.status !== "in_service" ? (
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
            ...
            {unit?.oos_at || unit?.oos_by ? (
              <p className="mt-1 text-sm font-semibold text-slate-500">
                Marked {unit.oos_at ? new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", month: "numeric", day: "numeric", year: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(unit.oos_at as string)) : ""}
                {unit.oos_by ? ` by ${unit.oos_by}` : ""}
              </p>
            ) : null}
          </div>
        ) : null}
              ...
              <article key={target.id} aria-label={`${target.name}: ${status}`} className={`rounded-3xl border-2 p-5 ${statusStyles[status]}`} role="status">
                ...
                {target.qrLocationNote ? (
                  <details className="mt-1">
                    ...
                    <p className="mt-1 text-sm font-semibold text-slate-700">{target.qrLocationNote}</p>
                  </details>
                ) : null}
              </article>
            );
