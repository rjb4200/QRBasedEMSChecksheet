...
The system SHALL identify a completed check by the presence of a `locked_at` timestamp on the crew record.

#### Scenario: Check with locked crew is completed
- **WHEN** a daily_unit_crews record has a non-null `locked_at` value
- **THEN** that check SHALL be considered completed

#### Scenario: Check without lock is not completed
- **WHEN** a daily_unit_crews record has a null `locked_at` value
...
- **THEN** the system SHALL use the most recent one (latest `locked_at` timestamp)
