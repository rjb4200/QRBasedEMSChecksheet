...
The system SHALL provide a `GET /api/admin/fleet-status` endpoint that returns aggregated fleet checkoff data as JSON for the current shift.

#### Scenario: Successful fleet status request
- **WHEN** an authenticated admin or supervisor requests `GET /api/admin/fleet-status`
...
- **WHEN** an unauthenticated client requests `GET /api/admin/fleet-status`
...
- **WHEN** `GET /api/admin/fleet-status` is called
