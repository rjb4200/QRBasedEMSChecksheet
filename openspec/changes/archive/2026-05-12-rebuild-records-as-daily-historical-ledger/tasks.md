...
- [x] 1.2 Update the Records query shape to load one selected operational date by default while preserving any required export/range entry points.
...
- [x] 2.1 Add a unit readiness state field with checked, incomplete, not started, and not required values.
- [x] 2.2 Classify out-of-service, archived, or otherwise not-required ledger rows as not required.
- [x] 2.3 Classify required units with no date-specific activity as not started.
- [x] 2.4 Classify required units with partial activity below the completion threshold as incomplete.
- [x] 2.5 Classify required units above the completion threshold as checked.
- [x] 2.6 Update summary totals to count readiness states and exclude not-required units from checked-over-required completion totals.

## 3. Records Page UI

- [x] 3.1 Rename and reframe the admin Records page copy as a daily historical readiness ledger.
- [x] 3.2 Replace the primary range-first summary interaction with a selected-date-first ledger view.
- [x] 3.3 Render every ledger-backed unit row for the selected date, including units with no archive/checkoff record.
- [x] 3.4 Display readiness state, unit status, snapshot/status note, crew names, comments, exceptions, timing, checked-by, and detail links where available.
- [x] 3.5 Show an explicit missing-ledger or best-effort indicator when the selected date has no ledger coverage.
