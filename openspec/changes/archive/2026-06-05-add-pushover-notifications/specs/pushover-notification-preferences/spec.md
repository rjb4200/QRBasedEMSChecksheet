...
The system SHALL store independent boolean preferences for daily report summary, missed checkoff alert, and missed checkoff follow-up alert.

#### Scenario: All event types enabled
- **WHEN** an admin has all three event type toggles enabled
- **THEN** the system SHALL send that admin all three types of Pushover alerts
...
- **AND** the system SHALL NOT send daily report or follow-up alerts to that admin
