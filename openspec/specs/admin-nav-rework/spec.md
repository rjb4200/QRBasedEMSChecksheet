...
The admin navigation SHALL place Units, Kits, Equipment, and Users inside a hamburger dropdown menu.

#### Scenario: Hamburger menu contains setup pages
- **WHEN** an admin opens the hamburger menu
...
Active page styling SHALL apply to both top-level links and menu links when the current page matches.
...
- **WHEN** the current page matches a menu link
...
#### Scenario: Mobile dropdown positions correctly
- **WHEN** the menu is opened on a mobile viewport
- **THEN** the dropdown SHALL not be clipped off-screen
